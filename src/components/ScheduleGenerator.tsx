...
const generateAssignments = (doctors: Doctor[], guardDays: GuardDay[], scheduleId: string, guardAssignments: GuardAssignment[], incompatibilities: any[]) => {
    ...
    doctors.forEach(doctor => {
      ...
      const lastGuardDate = doctorHistoricalStats.length > 0 ? doctorHistoricalStats.reduce((max, obj) => new Date(obj.date) > new Date(max.date) ? obj : max).date : null;
    ...
    incompatibilities.forEach(({ doctor_id, incompatible_doctor_id }) => {
      if (!incompatibilityMap.has(doctor_id)) incompatibilityMap.set(doctor_id, new Set());
      if (!incompatibilityMap.has(incompatible_doctor_id)) incompatibilityMap.set(incompatible_doctor_id, new Set());
      incompatibilityMap.get(doctor_id)!.add(incompatible_doctor_id);
      incompatibilityMap.get(incompatible_doctor_id)!.add(doctor_id);
    });
  
    let unassignedDays: { date: string, shiftType: ShiftType }[] = [];
      ...
      for (const day of days) {
        ...
        for (const shiftType of ['7h', '17h', '17h'] as ShiftType[]) {
          let eligibleDoctors = doctors.filter(doctor => {
            ...
            const hasIncompatible = Array.from(assignedOnDate).some(id => incompatibilityMap.get(doctor.id)?.has(id));
  
            return (
              !doctor.unavailable_weekdays.includes(dayOfWeek) &&
              (shiftType === '7h'
                ? stats['7h'] - stats['7h_init'] < getMaxPerDoctor(doctors.length, guardDays, '7h', false)
                : stats['17h'] - stats['17h_init'] < getMaxPerDoctor(doctors.length, guardDays, '17h', false)) &&
              diffDays >= 2 &&
          ...
          if (selected) {
            ...
            const shiftPosition = shiftType === '7h' ? 1 : ++shift17hCount;
            schedule.push({
    ...
    // Se puede agregar tercera y cuarta ronda como mejoras posteriores si aún hay unassignedDays
